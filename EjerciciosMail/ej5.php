...
sanear();
    ...
    if ($contfallos == 0) {
		...
		{	
            ...
            $temporales=$_FILES["archivo"]["tmp_name"];
            ...
            for($i=0;$i<count($temporales);$i++){
                if(is_uploaded_file($temporales[$i]))
                {
                     ...
                     $archivo = fopen($temporales[$i], "rb");
                     $datos = fread( $archivo, filesize($temporales[$i]) );
                     fclose($archivo);
			...
			$descripcion .= "--".$separador."\r\n";
			$descripcion .=	"Content-Type: ".$_FILES["archivo"]["type"][$i]."; name='".$_FILES["archivo"]["name"][$i]."'\r\n";
			$descripcion .= "Content-Transfer-Encoding: base64\r\n";
			$descripcion .= "Content-Disposition: attachment; filename='".$_FILES["archivo"]["name"][$i]."'\r\n\r\n";
			$descripcion .= $datos."\r\n\r\n";
